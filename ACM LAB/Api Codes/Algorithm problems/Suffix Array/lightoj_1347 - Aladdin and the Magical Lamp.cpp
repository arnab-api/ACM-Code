...
void printSA(int n)
{
    ...
    puts("");
    for(int i=0;i<n;i++) printf("%2d\n",LCP[i]);
    puts("");
    return ;
}
///LCP[i] = longest common prefix of ith and (i-1)th
///in sorted prefix

bool Check(int A,int B,int C,int a1,int a2,int b1,int b2,int c1,int c2)
{
    if((A>=a1 && A<=a2) && (B>=b1 && B<=b2) && (C>=c1 && C<=c2)) return true;
    if((A>=a1 && A<=a2) && (C>=b1 && C<=b2) && (B>=c1 && B<=c2)) return true;
    if((B>=a1 && B<=a2) && (A>=b1 && A<=b2) && (C>=c1 && C<=c2)) return true;
    if((B>=a1 && B<=a2) && (C>=b1 && C<=b2) && (A>=c1 && A<=c2)) return true;
    if((C>=a1 && C<=a2) && (A>=b1 && A<=b2) && (B>=c1 && B<=c2)) return true;
    if((C>=a1 && C<=a2) && (B>=b1 && B<=b2) && (A>=c1 && A<=c2)) return true;

    return false;
}

int solve(int n,int a1,int a2,int b1,int b2,int c1,int c2)
{
    int ret=0,k;
    for(int i=2;i<n;i++){
        k = min(LCP[i-1],LCP[i]);
        if(Check(SA[i-2],SA[i-1],SA[i],a1,a2,b1,b2,c1,c2)) ret = max(ret,k);
    }
    return ret;
}
...
int main()
{
    ...
    while(tcase--)
    {
        ...
        b=B.size();
        c=C.size();
        int ans = solve(text.size(),0,a-1,a+1,a+b,a+b+2,a+b+c+1);
