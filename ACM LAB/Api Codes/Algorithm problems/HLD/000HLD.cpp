...
vector<int>adj[M],Costs[M],conn[M],check[M];
...
void HLD(int curNode, int cost, int prev)
{
    ...
    baseArr[ptr++] = curNode;  /// assign the node information in baseArr
    check[chainNo].push_back(curNode);
...
int main()
{
    ...
    while(scanf("%d",&n)==1)
    {
        ...
        for(int i = 1 ; i <= chainNo; i++){
            printf("%d ----> ",i);
            for(int j=0;j<check[i].size();j++) printf(" %d(%d)",check[i][j],posInBase[check[i][j]]);
            printf("\n");
        }
        for(int i=0;i<n;i++){
            printf("Subtree of %d ===> ",i);
            int j,k;
            for(j=posInBase[i],k=0 ; k<subSize[i]; j++, k++) printf(" %d(%d)",baseArr[j],posInBase[baseArr[j]]);
            printf("\n");
        }
    }
    return 0;
